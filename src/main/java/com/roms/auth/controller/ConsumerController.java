...
public class ConsumerController {
	...
	@PutMapping("/users/{id}")
	@CachePut(key="#id")
	public ResponseEntity<ResponseDTO> updateName(@PathVariable String id,@RequestBody User user){
		ResponseDTO responseDto = userService.update(id, user);
        return ResponseEntity.ok(responseDto);
//		return new ResponseEntity<>(userService.update(id, user),HttpStatus.ACCEPTED);
	...
	public Page<User> pagination(@PathVariable int offset,@PathVariable int pageSize)
	{
		Page<User> a=userService.pagination(offset, pageSize);
	     return a;
